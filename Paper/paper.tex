...
We choose $n=m=1$, so for each encryption, we divide eight messages into four
registers.

\subsubsection{Packing the data into bitslice format}

Let $a,b,c,d,e,f,g,h$ be eight messages with subscripts denoting individual
bits. We first put these messages into four SIMD registers $V_0,V_1,V_2,V_3$:
...
value $N$. It swaps bits in $A$ masked by $M$ with bits masked by $M\ll N$ in
B using only three XOR-, one AND- and two shift operations.
...
The following operations put all of the first bits of each S-Box into $V_0$,
each second bit into $V_1$ and so forth and regroups these bits into bytes.

\begin{align*}
    &\text{SWAPMOVE}(V_0,V_1,0x5555\dots 55,1) &\text{SWAPMOVE}(V_0,V_0,0xf0f0\dots 0f,4) \\
    &\text{SWAPMOVE}(V_2,V_3,0x5555\dots 55,1) &\text{SWAPMOVE}(V_1,V_1,0xf0f0\dots 0f,4) \\
    &\text{SWAPMOVE}(V_0,V_2,0x3333\dots 33,2) &\text{SWAPMOVE}(V_2,V_2,0xf0f0\dots 0f,4) \\
    &\text{SWAPMOVE}(V_1,V_3,0x3333\dots 33,2) &\text{SWAPMOVE}(V_3,V_3,0xf0f0\dots 0f,4)
\end{align*}

With numbers denoting byte groups (i.e. $3$ corresponding to
$h_3g_3f_3e_3d_3c_3b_3a_3$), our data now has the following
permutation-friendly format:

\begin{alignat*}{7}
    V_0&=&&124&&||\dots&&||12&&||8 &&||4&&||0 \\
    V_1&=&&125&&||\dots&&||13&&||9 &&||5&&||1 \\
    V_2&=&&126&&||\dots&&||14&&||10&&||6&&||2 \\
    V_3&=&&127&&||\dots&&||15&&||11&&||7&&||3 \\
\end{alignat*}
...
to do some trivial rearrangements. Putting these values into registers
$V_5,V_6,V_7,V_8$ now allows us to compute the permutation layer in only four
instructions:

\begin{align*}
    &\texttt{TBL V0, V0, V5} &\texttt{TBL V1, V1, V6} \\
    &\texttt{TBL V2, V2, V7} &\texttt{TBL V3, V3, V8}
\end{align*}
